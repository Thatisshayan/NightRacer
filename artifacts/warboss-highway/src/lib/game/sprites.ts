...
const SPRITE_PACK_VERSION = 'v3';

// Runtime pack generated by scripts/build-sprite-pack.mjs from the authored
// art in artifacts/warboss-highway/assets-src/sprites-premium/ (which stays
// the source of truth and is not loaded at runtime). The authored vehicle art is 1373x2048; vehicles draw at
// 48x80 - 80x128 CSS px, so shipping the authored files meant a ~1:26 runtime
// minification, which bilinear sampling turns into aliased mush — the art
// looked worse *because* the source was large. The generated pack is Lanczos-
// resampled to ~2x max display size. See
// audits/2026-08-14_Viktor_UIUXVisual_Audit.md S3.
const RUNTIME_SPRITE_BASE = '/sprites';
...
export async function loadSpriteTextures(
  renderer: Renderer,
  base: string = RUNTIME_SPRITE_BASE
  ...
  // Vehicles still minify (a 256px-wide texture drawn at 48-80px), and a
  // minifying draw without a mip chain samples only a sparse subset of texels,
  // which shimmers under motion. Generating mipmaps lets the GPU pick a
  // pre-filtered level per fragment, so detail averages instead of aliasing.
  // Done once at load, not per frame.
  const mipmapped = new Set<Texture['source']>();
  for (const texture of Object.values(loaded) as Texture[]) {
    const source = texture?.source;
    if (!source || mipmapped.has(source)) continue;
    mipmapped.add(source);
