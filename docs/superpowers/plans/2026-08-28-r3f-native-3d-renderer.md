...
**Not done — genuinely unverified, not just unchecked as a formality:**
- `pnpm install` for the new dependencies was still resolving in the background when this session's turn ended (this environment's pnpm resolution has been extremely slow all session — 15–20+ min per full install). **No typecheck has passed against the new code yet.**
- Nothing in this plan has run on an iOS simulator or device. This session has no macOS host — `expo run:ios` isn't possible here. CI's macOS runner (`.github/workflows/ios-build.yml`) is the realistic place this gets its first real verification, same as the existing iOS pipeline.
...
- Neon billboards, tunnel biome geometry, and real GLTF vehicle models remain deferred as originally scoped.

**Before this branch is worth pushing/reviewing:** finish the `pnpm install`, run `pnpm --filter @workspace/warboss-highway-mobile run typecheck` (fix whatever the first real typecheck run surfaces — none of this has been type-checked yet), and get an actual on-device or CI build to confirm the scene renders at all.
