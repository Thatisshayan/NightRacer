...
## Items
- [2026-09-01] **Native R3F mobile renderer — drivers draw no ground shadows/reflections** - `<primitive>` GLTF vehicle children in `VehicleMesh.tsx` have no `castShadow`/`receiveShadow`, and `VehicleMesh.update` ignores `ApexVehiclePose.shadow`/`.reflection`, so cars float with no ground contact shading on the native 3D renderer (matches the reported "wrong shadows/lighting") - deferred because a proper fix (per-vehicle shadow/reflection planes driven by the pose fields) plus verification really needs an on-device/simulator render to confirm placement/framing, which this Windows-only session cannot do - resume by adding shadow/reflection planes to `VehicleMesh` keyed off `pose.shadow`/`pose.reflection`, then verify on `.github/workflows/ios-build.yml` CI artifact or a device - status: open.
- [2026-09-01] **Native R3F mobile renderer — PostFX Bloom on expo-gl artifact source** - `PostFX.tsx` `EffectComposer` + `<Bloom mipmapBlur />` drives half-float/MRT render targets through `@react-three/postprocessing` on expo-gl's acknowledged-incomplete WebGL2 (see `Atmosphere.tsx` comment); per the on-device "garbage pixels / wrong colors" report and the proven-good no-composer smoke-test baseline, this is now **gated OFF by default** (PostFX returns null unless `EXPO_PUBLIC_ENABLE_POSTFX=1`) to remove the artifact class without device logs - status: mitigated (re-enable + validate on a device before turning it back on).
- [2026-09-01] **Native R3F mobile renderer — `file://` GLTF load crash** - `LoadedGltf` loaded via `useLoader(GLTFLoader, file://uri)`; on a real device GLTFLoader's internal `fetch()` rejects `file://`, `useLoader` rethrows into the root `ErrorBoundary` → the "something went wrong, please reload" crash (matches the on-device "crash/console error" report). Fixed in PR #53 by replacing `useLoader` with an imperative `GLTFLoader.loadAsync` + try/catch: on success renders the normalized model as before; on rejection renders a grounded placeholder (car-shaped, y=0) and caches the failed URI, so a load can never crash the app and no slot goes blank - status: mitigated-soft (real GLTF models may not render on device until the `file://` fetch issue is solved properly; placeholder shows in that case).
- [2026-09-01] **Native R3F mobile renderer — vehicle forward-orientation still unconfirmed** - each single-image-to-3D GLB's "forward" axis vs the +Z this code applies via `rotation.y` was never visually verified (plan Task-4 note). The road no longer requires PIXI_RENDERER-facing math after the camera-aim fix, but a model could still face/drive the wrong way - deferred because it needs an actual render to observe - resume by confirming each model's heading on a real device and, if wrong, adding a per-model forward rotation offset constant - status: open.
- [2026-09-01] **Native R3F mobile renderer — per-frame `setActiveKey` churn** - `VehicleMesh.update` calls `setActiveKey(...)` on every `useFrame` tick for every active slot (up to 10 setState calls/frame); React bails on identical values but it is needless hot-path state churn on a phone - deferred as a perf/cleanliness item, not a correctness bug - resume by only calling `setActiveKey` when the resolved key actually differs from the current one - status: open.
- [2026-09-01] **Local `node_modules` is stale/incomplete — bare `tsc` in a fresh checkout reports hundreds of false "Cannot find module" errors** - running `tsc -p tsconfig.json` directly for `@workspace/warboss-highway-mobile` without a completed install showed ~484 errors (`Cannot find module 'three'/'expo-asset'/'@workspace/*'`, `--jsx is not set`, missing `JSX.IntrinsicElements`): all are a stale-`node_modules` cascade, NOT source bugs. The deps are declared in `package.json` and present in pnpm's store (`.pnpm`) but not linked into the workspace `node_modules` tree. Confirmed non-issue: CI (`gate` + `ios-build`) runs `pnpm install --frozen-lockfile` and the identical mobile files compile/archive clean (ios-build 13m46s pass on PR #51 / `main` at `dbd57ce`). A local `pnpm install --frozen-lockfile` fixes it, but is network-bound (~1h in this environment) so it was deferred rather than run to completion - resume by letting `pnpm install` finish locally if a green offline typecheck is wanted; otherwise trust CI - status: open (dependency note, no code change).
...
- [2026-08-27] Vehicle grounding and cyberpunk atmosphere release validation: physical-device native and hardware-WebGL profiling of projected wheel contact, lightning, rain, billboard reflection, dense traffic, and Rush — deferred because this execution environment has no attached Android/iOS device, GPU frame profiler, or hardware-WebGL visual capture; typechecks, repository gate, and a 420×800 isolated-browser gameplay smoke passed, but that browser is not device/hardware certification — resume with a release build on representative low/mid/high devices; measure FPS, JS/UI/render-thread time, GPU/frame time where available, memory/texture residency, and headlight readability during lightning — status: implemented, runtime certification unverified.
- [2026-08-27] Pixi CanvasRenderer and explicit Canvas2D fallback parity: grounded-contact, lightning, and billboard-reflection substitutions — deferred because this pass corrects the Pixi and native Skia presentation paths; CanvasRenderer can omit GPU-only effects and the separate Canvas2D fallback retains its own renderer — resume by explicitly selecting each fallback backend, confirming vehicle contact/readability, and documenting any intentionally reduced atmosphere treatment — status: open, backend-specific follow-up.
- [2026-08-28] Apex Storm composition candidate (`feat/apex-storm-renderer`): deterministic 420×800 web-first Babylon proof captured at `audits/2026-08-28_Manus_ApexStormComposition_Proof.png`; verified and advanced to motion. — **status: completed.**
- [2026-08-28] Apex Storm motion proof (`feat/apex-storm-renderer`): real-time web gameplay motion integration, high-speed FOV effects, and collision feedback captured at `audits/2026-08-28_Manus_ApexStormMotion_Proof.png` and `audits/2026-08-28_Manus_ApexStormRush_Proof.png`; verified and advanced to assets. — **status: completed.**
- [2026-08-28] Apex Storm visual assets (`feat/apex-storm-renderer`): high-quality cyberpunk vehicle textures and removal of legacy procedural artifacts captured at `audits/2026-08-28_Manus_ApexStormAssets_Proof.png`; verified and advanced to atmosphere. — **status: completed.**
- [2026-08-28] Apex Storm atmosphere and effects (`feat/apex-storm-renderer`): cinematic rain, deterministic lightning, and neon billboard reflections captured at `audits/2026-08-28_Manus_ApexStormRain_Proof.png` and `audits/2026-08-28_Manus_ApexStormLightning_Proof.png`; verified and advanced to biomes. — **status: completed.**
- [2026-08-28] Neon Tunnel Run district (`feat/apex-storm-renderer`): concrete road textures, overhead architectural geometry, and steam effects captured at `audits/2026-08-28_Manus_NeonTunnelRun_Proof.png`; the biome is a candidate awaiting Shayan’s visual acceptance — deferred because the approved contract requires owner review of biomes before native parity — resume by reviewing the tunnel capture against the visual contract; only after acceptance, add Skyline Siege events, native Skia adaptation, and device testing — **status: awaiting owner visual review.**
- [2026-08-28] Apex Storm Babylon runtime validation: software-WebGL Chromium initialized and rendered the composition proof, but the isolated Playwright Firefox environment reported `WebGL not supported` at Babylon `Engine` initialization; no physical iOS/Android device, hardware WebGL capture, or GPU profiler was available — deferred because software rasterization validates scene construction but cannot certify hardware fidelity, performance, or mobile parity — resume with representative low/mid/high device release builds; measure FPS, GPU/frame time, memory/texture residency, controller/input behavior, vehicle contact readability, and traffic legibility in real motion — **status: runtime certification unverified.**
- [2026-08-28] Apex Storm release path: the feature branch remains intentionally opt-in behind `?renderer=apex`; Pixi stays default and Canvas2D/native paths are preserved — deferred because default routing, merge, iOS archive, and TestFlight require explicit visual acceptance followed by separate motion/device gates and owner authorization — resume only after those gates are recorded as passed in a subsequent audit/PR — **status: intentionally not started.**
- [2026-08-31] Regenerate TypeScript declarations for `lib/game-core`: The declaration files in `lib/game-core/dist/` are outdated and were manually patched as a temporary fix. Run `pnpm run typecheck:libs` at the workspace root to regenerate declarations permanently — **status: open, not started.**
- [2026-08-31] On-device verification for iOS fixes: The renderer desync fix (scale dependency in `GameCanvas.tsx`), framerate independence, and Rush mechanics (`triggerRush`, `rushTimer`, `rushCharge`) need to be verified on a physical iOS device or simulator — **status: open, not started.**
- [2026-08-31] Leaderboard audit: Verify score submission logic in `GameOverScreen.tsx` and server-side plausibility checks (e.g., `MAX_SCORE_PER_DISTANCE` in `api-server`) to ensure leaderboard integrity — **status: open, not started.**
- [2026-08-31] App Store Optimization (ASO): Implement keywords, localize metadata, and create screenshots/preview videos to drive adoption and achieve top-10 ranking in Arcade/Racing categories — **status: open, not started.**
