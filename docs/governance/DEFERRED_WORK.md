...
- [2026-09-01] **Native R3F mobile renderer — PostFX Bloom on expo-gl unverified / likely artifact source** - `PostFX.tsx` `<Bloom mipmapBlur />` drives half-float/MRT render targets through `@react-three/postprocessing`; expo-gl's WebGL2 support is acknowledged incomplete (see `Atmosphere.tsx` comment), and Bloom's mipmap blur is a classic source of colored-noise/garbage pixels on mobile GL drivers. Never validated on a device - deferred because proving/fixing requires a real GL device to reproduce, and it risks frame rate if left enabled - resume by rendering the scene on a device/simulator with and without `<EffectComposer>`, and disable Bloom (or gate it) if it produces artifacts or drops frame rate - status: open.
